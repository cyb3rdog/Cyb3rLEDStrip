...
class BouncingBallEffect
{
  ...
  public:
          ...
          _bMirrored(bMirrored)         
    {
        ...
        Colors = new CRGB[ballCount];

        for (size_t i = 0; i < ballCount; i++)
        {
            Height[i]                = StartHeight;                 // Current Ball Height
            ClockTimeAtLastBounce[i] = millis();                    // When ball last hit ground state
    ...
    virtual void Draw()
    {
        ...
        {
            double TimeSinceLastBounce = (millis() - ClockTimeAtLastBounce[i]) / SpeedKnob;
            ...
            if (Height[i] < 0)
            {
                ...
                ClockTimeAtLastBounce[i] = millis();
