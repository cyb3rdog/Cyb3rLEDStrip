...
uint8_t checkButton(int pin) {
   ...
   // Button pressed down
   if (buttonVal == LOW && buttonLast == HIGH && ((long)millis() - upTime) > debounce)
   {
       downTime = millis();
       ignoreUp = false;
       waitForUp = false;
       ...
       if ((long)(millis()-upTime) < DCgap && DConUp == false && DCwaiting == true)  DConUp = true;
       else  DConUp = false;
       DCwaiting = false;
   }
   // Button released
   ...
   if ( buttonVal == HIGH && ((long)millis()-upTime) >= DCgap && DCwaiting == true && DConUp == false && singleOK == true && event != 2)
   {
       event = 1;
       DCwaiting = false;
   }
   // Test for hold
   if (buttonVal == LOW && ((long)millis() - downTime) >= holdTime) {
       // Trigger "normal" hold
